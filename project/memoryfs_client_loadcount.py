...
class DiskBlocks():
  def __init__(self,N,argv):
    self.N=N
    self.block_server=[]
    self.load=np.zeros((1,N))
    for i in range (0,N):
        #print (N,argv[2+i])
        self.block_server.append(xmlrpc.client.ServerProxy(argv[2+i], use_builtin_types=True))
    print(self.block_server)    
    
    self.TOTAL_VIRTUAL_BLOCKS=(N-1)*TOTAL_NUM_BLOCKS
# Number of blocks needed for free bitmap
# For simplicity, we assume each entry in the bitmap is a Byte in length
# This allows us to avoid bit-wise operations
    self.FREEBITMAP_NUM_BLOCKS = self.TOTAL_VIRTUAL_BLOCKS // BLOCK_SIZE

# inode table starts at offset 2 + FREEBITMAP_NUM_BLOCKS
    self.INODE_BLOCK_OFFSET = 2 + self.FREEBITMAP_NUM_BLOCKS
    self.DATA_BLOCKS_OFFSET = self.INODE_BLOCK_OFFSET + INODE_NUM_BLOCKS
    self.DATA_NUM_BLOCKS = self.TOTAL_VIRTUAL_BLOCKS - self.DATA_BLOCKS_OFFSET
#    self.block=self.block_server.block
...
## Blocks are padded with zeroes up to BLOCK_SIZE
  ...
  def VirtualBlockNumberToServerBlockNumber(self, virtual_block_number):
    server_id=(virtual_block_number)%(self.N-1)
    block_number=math.floor((virtual_block_number)/(self.N-1))
    parity_id=self.N-1-(block_number%self.N)
    if server_id>=parity_id:
        server_id=(server_id+1)%(self.N)
    #print (server_id,block_number,parity_id)
    return server_id,block_number,parity_id
  
 
  def Put(self, virtual_block_number, block_data):
    logging.debug ('Put: block number ' + str(virtual_block_number) + ' len ' + str(len(block_data)) + '\n' + str(block_data.hex()))
    if len(block_data) > BLOCK_SIZE:
      logging.error('Put: Block larger than BLOCK_SIZE: ' + str(len(block_data)))
      quit()

    if virtual_block_number in range(0,self.TOTAL_VIRTUAL_BLOCKS): 
      # ljust does the padding with zeros
      putdata = bytearray(block_data.ljust(BLOCK_SIZE,b'\x00'))
      server_id,block_number,parity_id=self.VirtualBlockNumberToServerBlockNumber(virtual_block_number)
      #print (virtual_block_number,server_id,block_number,parity_id)
      # print(self.block_server[server_id].Get(block_number)[0])
      old_data=self.SingleGet(server_id,block_number)
      
      old_parity=self.SingleGet(parity_id,block_number)
      parity_data=bytearray(self.Xor(self.Xor(old_data,putdata),old_parity))
      #print(len(old_data),len(old_parity),len(parity_data))
      #print(type(parity_data),type(putdata))
      #parity_data=bytearray(self.Xor(self.Xor(old_data,putdata),bytearray(self.block_server[parity_id].Get(block_number)[0])))
      try:
          self.block_server[server_id].Put(block_number,putdata)
          self.load[0,server_id]+=1
      except socket.error:
          print ('Put: server '+str(server_id)+' is failed')
      try:
          self.block_server[parity_id].Put(block_number,parity_data)
          self.load[0,server_id]+=1
          print (self.load)
      except socket.error:
          print ('Put: server '+str(parity_id)+' is failed')
      # Write block
...
## Equivalent to the textbook's BLOCK_NUMBER_TO_BLOCK(b)
  ...
  def SingleGet(self,server_id,block_number):
      try:
          block_data,Flag=self.block_server[server_id].Get(block_number)
          block_data=bytearray(block_data)
          if Flag:
              print ('Get: block '+str(block_number)+' is damaged')
              block_data=self.GetFromParity(server_id,block_number)
      except socket.error:
          print ('Get: server '+str(server_id)+' is failed')
          block_data=self.GetFromParity(server_id,block_number)
...
#      return self.block[block_number]
      ...
      try:
          ...
          print ('Get: server '+str(server_id)+' is failed')
          block_data=self.GetFromParity(server_id,block_number)
      #print(Flag,server_id,block_number,parity_id)
      
      self.load[0,server_id]+=1
      print(self.load)
      return block_data

    logging.error('Get: Block number larger than TOTAL_VIRTUAL_BLOCKS: ' + str(block_number))
    quit()
 
  def GetFromParity(self,server_id,block_number):
    print('Recover from parity')
    if server_id==0:
        block_data=self.SingleGet(1,block_number)
        iStart=2
    else:
        block_data=self.SingleGet(0,block_number)
        iStart=1
    #print (block_data,type(block_data))
    for i in range (iStart,self.N):
        if (i!=server_id):
            #print (block_data,self.SingleGet(i,block_number))
            block_data=self.Xor(block_data,self.SingleGet(i,block_number))
            #print (block_data)
#    block_data=bytearray(block_data)
    return block_data
...
## Serializes and saves block[] data structure to a disk file
  ...
  def DumpToDisk(self, prefix):
    ...
    filename = str(prefix.hex()) + "_BS_" + str(BLOCK_SIZE) + "_NB_" + str(self.TOTAL_VIRTUAL_BLOCKS) + "_IS_" + str(INODE_SIZE) + "_MI_" + str(MAX_NUM_INODES) + ".dump"
...
## Loads block[] data structure from a disk file
  ...
  def LoadFromDisk(self, prefix):
    ...
    for i in range(0, self.TOTAL_VIRTUAL_BLOCKS):
      self.Put(i,block[i])
    file.close()
...
#        server_id,block_number,parity_id=self.VirtualBlockNumberToServerBlockNumber(i)
#        self.block_server[server_id].Put(block_number,zeroblock)
#        self.block_server[parity_id].Put(block_number,zeroblock)
      # Block 1: Superblock contains basic file system constants
      # First, we write it as a list
      superblock = [self.TOTAL_VIRTUAL_BLOCKS, BLOCK_SIZE, MAX_NUM_INODES, INODE_SIZE]
      # Now we serialize it into a byte array
      self.Put(1,pickle.dumps(superblock))

      # Blocks 2-TOTAL_VIRTUAL_BLOCKS are initialized with zeroes
...
## Prints out file system information
  ...
  def PrintFSInfo(self):
    ...
    logging.info ('Number of blocks          : ' + str(self.TOTAL_VIRTUAL_BLOCKS))
    ...
    logging.info ('Free bitmap size (blocks) : ' + str(self.FREEBITMAP_NUM_BLOCKS))
    logging.info ('Inode table offset        : ' + str(self.INODE_BLOCK_OFFSET))
    logging.info ('Inode table size (blocks) : ' + str(INODE_NUM_BLOCKS))
    logging.info ('Max blocks per file       : ' + str(MAX_INODE_BLOCK_NUMBERS))
    logging.info ('Data blocks offset        : ' + str(self.DATA_BLOCKS_OFFSET))
    logging.info ('Data block size (blocks)  : ' + str(self.DATA_NUM_BLOCKS))
    logging.info ('Raw block layer layout: (B: boot, S: superblock, F: free bitmap, I: inode, D: data')
    Layout = "BS"
    Id = "01"
    IdCount = 2
    for i in range(0,self.FREEBITMAP_NUM_BLOCKS):
      Layout += "F"
      Id += str(IdCount)
    ...
    for i in range(0,INODE_NUM_BLOCKS):
      ...
      Id += str(IdCount)
      IdCount = (IdCount + 1) % 10
    for i in range(0,self.DATA_NUM_BLOCKS):
      Layout += "D"
      Id += str(IdCount)
      IdCount = (IdCount + 1) % 10
    logging.info (Id)
...
## The inode data structure loaded from raw storage goes in the self.inode object
  ...
  def InodeNumberToInode(self):
    ...
    raw_block_number = self.RawBlocks.INODE_BLOCK_OFFSET + ((self.inode_number * INODE_SIZE) // BLOCK_SIZE)
...
## Allocate a data block, update free bitmap, and return its number
  ...
  def AllocateDataBlock(self):
    ...
    for block_number in range ( self.RawBlocks.DATA_BLOCKS_OFFSET, self.RawBlocks.TOTAL_VIRTUAL_BLOCKS):
